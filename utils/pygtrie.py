...
# common detect function
def trie_match(text, tree, shallow=True):
    """通用字典树匹配逻辑
    [IN]  text: string, 待匹配字符串
          tree: CharTrie, 字典树
          shallow: boolean, true则命中一个即返回
    [OUT] res_list: list[(key, value)], 命中结果列表
    """
    res_list = list()
    cur_text = ""
    for word in text[::-1]:
        cur_text = word + cur_text
        key, value = tree.longest_prefix(cur_text)
        if key is not None:
